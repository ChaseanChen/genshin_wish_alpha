...
try:
    ...
    # df_character_original 和 five_character_stars 未被后续代码使用，可以考虑删除以简化。
    # df_character_original = pd.DataFrame(test_df_character)
...
try:
    ...
    for original_sheet_name, df in all_sheet_data.items():
        ...
        if column_to_convert in df.columns:
            # 修改点：
            # 1. 筛选出 'name' 列的值在 list_map 键中的行
            # .copy() 是为了避免 SettingWithCopyWarning，确保操作在一个独立的副本上
            df_filtered = df[df[column_to_convert].isin(list_map.keys())].copy()

            # 2. 对筛选后的 DataFrame 进行名称转换
            # 因为已经筛选过，所以 df_filtered 中所有的 'name' 都肯定在 list_map 中，直接 replace 即可
            df_filtered.loc[:, column_to_convert] = df_filtered[column_to_convert].replace(list_map)

            # 将处理后的 DataFrame 添加到 converted_sheets
            converted_sheets[new_sheet_name] = df_filtered
        else:
            # 如果工作表没有 'name' 列，则无法进行名称转换和行删除
            # 根据需求，这里可以有两种处理方式：
            # a) 完全跳过这个sheet (不添加到 converted_sheets)
            # b) 保持这个sheet原样 (将其添加到 converted_sheets，但没有进行名称和行操作)
            # 当前代码选择 b)
